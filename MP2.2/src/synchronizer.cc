...
void Heartbeat(std::string coordinatorIp, std::string coordinatorPort,
  ...
  std::unique_ptr<CoordService::Stub> stub =
      std::unique_ptr<CoordService::Stub>(
          CoordService::NewStub(grpc::CreateChannel(
              coordinatorInfo, grpc::InsecureChannelCredentials())));

  // send a heartbeat to the coordinator, which registers your follower
  // synchronizer as either a master or a slave
...
std::vector<std::string> get_all_users_func(int synchID) {
  ...
  // std::string master_users_file =
  // "./master"+std::to_string(synchID)+"/all_users"; std::string
  // slave_users_file = "./slave"+std::to_string(synchID)+"/all_users";
  ...
  std::vector<std::string> master_user_list =
      get_lines_from_file(master_users_file);
  std::vector<std::string> slave_user_list =
      get_lines_from_file(slave_users_file);
...
std::vector<std::string> get_tl_or_fl(int /*synchID*/, int clientID, bool tl) {
  ...
  std::string master_fn =
      "cluster_" + std::to_string(clusterID) + "/1/" + std::to_string(clientID);
  std::string slave_fn =
      "cluster_" + std::to_string(clusterID) + "/2/" + std::to_string(clientID);
  if (tl) {
...
std::vector<std::string> getFollowersOfUser(int ID) {
  ...
  for (auto userID : usersInCluster) {  // Examine each user's following file
    std::string file = "cluster_" + std::to_string(clusterID) + "/" +
                       clusterSubdirectory + "/" + userID + "_follow_list.txt";
    std::string semName = "/" + std::to_string(clusterID) + "_" +
                          clusterSubdirectory + "_" + userID +
                          "_follow_list.txt";
    sem_t *fileSem = sem_open(semName.c_str(), O_CREAT);
